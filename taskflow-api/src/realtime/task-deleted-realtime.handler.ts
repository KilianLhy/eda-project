...
@Injectable()
export class TaskDeletedRealtimeHandler implements OnModuleInit {
  constructor(
  ...
  onModuleInit(): void {
    ...
    this.eventBus.subscribe('task.deleted', (event) =>
      this.handleTaskDeleted(event),
    );
  }

  private handleTaskDeleted(event: DomainEvent): void {
    const projectId =
    ...
    this.realtimeBroadcaster.broadcastToProject(projectId, 'task.deleted', {
